...
def simple_model():
    ...
    return SemanticModel(
        table=table,
        dimensions={
            "col_test": lambda t: t.col_test,
            "val": lambda t: t.val
        },
        measures={
            "sum_val": lambda t: t.val.sum(),
            "count": lambda t: t.val.count()
        }
    )
    ...
    [
        ...
        (
            ...
            pd.DataFrame({
                ...
                "total_amount": [400, 400]  # 400 for customer 101 (100+300) and 400 for customer 103
            }).rename(columns={
...
def test_filters(test_id, model_fixture, filters, dims, measures, expected_data, expected_error, request):
    """
    ...
    if expected_error is not None:
        ...
        result = expr.execute().sort_values([dim.replace(".", "_") for dim in dims]).reset_index(drop=True)
        expected = expected_data.sort_values([dim.replace(".", "_") for dim in dims]).reset_index(drop=True)
        pd.testing.assert_frame_equal(result, expected)
...
def test_json_filters_with_joins():
    ...
    assert int(result["total_amount"].sum()) == 900  # Sum of orders for US gold customers
...
def test_time_range_with_other_filters(time_model):
    ...
    assert result["total_value"].iloc[0] < total_value  # March value should be less than total
...
def test_invalid_time_range():
    ...
    # Test missing start
    with pytest.raises(ValueError, match="time_range must be a dictionary with 'start' and 'end' keys"):
        model.query(
            dims=["date"],
            measures=["total_value"],
...
def test_time_grain_validation():
    ...
    for grain in ["TIME_GRAIN_DAY", "TIME_GRAIN_MONTH", "TIME_GRAIN_QUARTER", "TIME_GRAIN_YEAR"]:
        expr = model.query(
            dims=["date"],
            measures=["total_value"],
            time_grain=grain
    ...
    for grain in ["TIME_GRAIN_HOUR", "TIME_GRAIN_MINUTE", "TIME_GRAIN_SECOND"]:
        with pytest.raises(ValueError, match="is finer than the smallest allowed grain"):
            model.query(
                dims=["date"],
                measures=["total_value"],
                time_grain=grain
...
def test_time_grain_with_other_dimensions(time_model):
    ...
    monthly_categories = result_df.groupby(pd.to_datetime(result_df["date"]).dt.month)["category"].nunique()
