...
flights = ibis.memtable(
    {"origin": ["JFK", "LAX"], "destination": ["SFO", "ORD"], "distance": [2500, 300]}
)
...
assert graph2["origin_upper"]["deps"]["origin_code"] == "dimension", (
    "origin_upper should depend on dimension"
)
...
assert graph3["total_distance"]["deps"]["distance"] == "dimension", (
    "measure should depend on dimension (extended table)"
)
