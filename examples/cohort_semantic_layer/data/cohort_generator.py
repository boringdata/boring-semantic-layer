...
class DataGenerator:
    def __init__(self, start_date: str, end_date: str):
        ...
        self.customer_lifespan_months = (1, 18)  # Min/max months a customer stays active
        self.monthly_churn_rate = 0.15  # 15% of customers churn each month
        self.new_customer_rate_range = (0.15, 0.45)  # 15-45% new customers each month (random)
        
        # Order patterns
        self.orders_per_customer_per_month = (0.25, 2.0)  # Min/max orders per active customer (50% reduction)
    ...
    def generate_customers_for_month(self, year: int, month: int) -> List[str]:
        ...
        new_customer_count = random.randint(
            max(1, new_customer_count - variation),
            new_customer_count + variation
        )
    ...
    def generate_orders_for_month(self, year: int, month: int):
        ...
        for customer_id in self.active_customers:
            ...
            for _ in range(orders_count):
                order_date = current_date + timedelta(days=random.randint(0, days_in_month - 1))
    ...
    def generate_data(self):
        ...
        print(f"Generating data from {self.start_date.strftime('%Y-%m-%d')} to {self.end_date.strftime('%Y-%m-%d')}")
    ...
    def save_orders_csv(self, filename: str = "orders.csv"):
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['order_id', 'order_date', 'order_amount', 'customer_id', 'product_count']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python cohort.py --start-date 2023-01-01 --end-date 2023-12-31
  python cohort.py -s 2024-01-01 -e 2024-06-30
  python cohort.py --start-date 2023-01-01 --end-date 2024-12-31 --customers-file my_customers.csv
        """
    )
    ...
    # Save files
    generator.save_customers_csv(args.customers_file)
    generator.save_orders_csv(args.orders_file)
    ...
    print(f"  - {args.customers_file}")
    print(f"  - {args.orders_file}")
