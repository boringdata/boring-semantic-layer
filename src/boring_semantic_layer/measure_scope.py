...
@curry
def _resolve_measure_name(name: str, known: tuple[str, ...], known_set: frozenset[str]) -> str | None:
    return (name if name in known_set
            else next((k for k in known if k.endswith(f".{name}")), None))


def _make_known_measures(measures: Iterable[str]) -> tuple[tuple[str, ...], frozenset[str]]:
