...
def _lower_semantic_filter(node: SemanticFilter, catalog, *args):
    ...
    if isinstance(node.source, SemanticAggregate):
        ...
        if len(all_roots) > 1:  # Joined table
            ...
            dim_map = (all_roots[0]._dims_dict() if hasattr(all_roots[0], '_dims_dict') else all_roots[0].dimensions) if all_roots else {}
...
def _lower_semantic_project(node: SemanticProject, catalog, *args):
    ...
    if len(all_roots) > 1:  # Joined table
        ...
        merged_dimensions = (all_roots[0]._dims_dict() if hasattr(all_roots[0], '_dims_dict') else all_roots[0].dimensions) if all_roots else {}
        merged_measures = (all_roots[0]._measures_dict() if hasattr(all_roots[0], '_measures_dict') else all_roots[0].measures) if all_roots else {}
