...
QUERIES = {"comparing_timeframe": ["query_1"]}
import sys
...
def test_malloy_queries():
    for query_file, queries in QUERIES.items():
        for query_name in queries:
            ...
            df_malloy_sorted = df_malloy.sort_values(by=common_cols).reset_index(
                drop=True
            )
            df_bsl_sorted = (
                df_bsl[common_cols].sort_values(by=common_cols).reset_index(drop=True)
            )

            if not df_malloy_sorted.equals(df_bsl_sorted):
                print(f"\nDifference found in {query_file}.{query_name}:")
                print(
                    f"Shape - Malloy: {df_malloy_sorted.shape}, BSL: {df_bsl_sorted.shape}"
                )
                ...
                try:
                    ...
                    if len(df_malloy_sorted) == len(df_bsl_sorted):
                        ...
                        malloy_not_in_bsl = (
                            df_malloy_sorted.merge(
                                df_bsl_sorted, how="left", indicator=True
                            )
                        ...
                        bsl_not_in_malloy = (
                            df_bsl_sorted.merge(
                                df_malloy_sorted, how="left", indicator=True
                            )
                            .query('_merge == "left_only"')
                            .drop("_merge", axis=1)
                        )
