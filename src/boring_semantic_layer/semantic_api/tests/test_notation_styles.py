...
class TestDictBasedMetadata:
    """Test the new dict-based API for dimensions and measures with metadata."""
    ...
    def test_dimension_with_metadata(self, flights_data):
        ...
        assert st._dims["carrier"].description == "Carrier code"
        assert st._dims["distance"].description == "Flight distance in miles"
        assert st._dims["distance"].is_time_dimension is False
    ...
    def test_measure_with_metadata(self, flights_data):
        ...
        # Verify metadata is stored
        assert st._base_measures["flight_count"].description == "Total number of flights"
        assert st._base_measures["total_distance"].description == "Sum of all flight distances"
    ...
    def test_mixed_callable_and_dict(self, flights_data):
        ...
        assert st._dims["carrier"].description is None
        # Dict should have description
        assert st._dims["distance"].description == "Distance traveled"

        # Same for measures
        assert st._base_measures["flight_count"].description is None
        assert st._base_measures["avg_distance"].description == "Average flight distance"
    ...
    def test_descriptions_preserved_through_operations(self, flights_data):
        """Test that descriptions are preserved through various operations."""
        tbl = flights_data["flights"]
        carriers_tbl = flights_data["carriers"]
        ...
        carriers_st = (
            to_semantic_table(carriers_tbl, "carriers")
            .with_dimensions(
                code={
                    "expr": lambda t: t.code,
                    "description": "Carrier code for joining"
                }
            )
        )

        # Test 1: Descriptions preserved after filter
        filtered = flights_st.filter(lambda t: t.carrier == "AA")
        assert filtered._dims["carrier"].description == "Airline carrier code"
        assert filtered._base_measures["flight_count"].description == "Total number of flights"

        # Test 2: Descriptions preserved after aggregate
        aggregated = (
            flights_st
            .group_by("carrier")
            .aggregate("flight_count", "total_distance")
        )
        # After aggregation, dimensions should be preserved
        assert aggregated._dims["carrier"].description == "Airline carrier code"
        # Note: measures are not preserved after aggregation (they become columns)

        # Test 3: Descriptions preserved after join
        joined = flights_st.join_one(carriers_st, "carrier", "code")
        # Joined dimensions should have prefixed names
        assert joined._dims["flights__carrier"].description == "Airline carrier code"
        assert joined._dims["carriers__code"].description == "Carrier code for joining"
        assert joined._base_measures["flights__flight_count"].description == "Total number of flights"

    def test_time_dimension_metadata_preserved(self, flights_data):
        ...
        assert st._dims["carrier"].is_time_dimension is False
        assert st._dims["carrier"].smallest_time_grain is None
        assert st._dims["distance"].is_time_dimension is False
        assert st._dims["distance"].smallest_time_grain is None

        # After filter, metadata should be preserved
        filtered = st.filter(lambda t: t.carrier == "AA")
        assert filtered._dims["carrier"].is_time_dimension is False
        assert filtered._dims["distance"].is_time_dimension is False
