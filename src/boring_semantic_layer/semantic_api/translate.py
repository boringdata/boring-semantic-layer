...
@convert.register(ops.Limit)
@convert.register(Limit)
def convert_limit(limit, catalog):
    # Vendor Ibis Limit op: return the table expression as-is
    if not hasattr(limit, "input"):
        return limit.to_expr()

    # DataFusion plan Limit: lower to Ibis limit
