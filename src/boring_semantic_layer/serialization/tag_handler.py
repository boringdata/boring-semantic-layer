...
def reemit(tag_node, rebuild_subexpr):
    ...
    xorq's dispatch only routes here when the registry lookup on
    ``tag_node.metadata["tag"]`` resolves to this handler, and xorq's own
    op definition declares ``parent: Relation`` (non-null) — so by
